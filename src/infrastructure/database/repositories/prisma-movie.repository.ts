  ...
  async search(filters: SearchMoviesDto): Promise<Movie[]> {
    const where: Prisma.MovieWhereInput = {
      ...
      category: filters.category
        ? this.toPrismaCategory(filters.category as MovieCategory)
        : undefined,
...
//       duration: movie.duration,
//       coverImage: movie.coverImage,
//       category: movie.category as MovieCategory, // Assure que le type correspond à l'entité du domaine
//       releaseDate: movie.releaseDate,
//       rating: movie.rating,
//     });
//   }
...
//   async create(movieData: Omit<Movie, 'id'>): Promise<Movie> {
//     const created = await this.prisma.movie.create({
//       data: { ...movieData, category: movieData.category as any }, // Prisma attend une string pour le champ category
//     });
//     return this.mapToDomain(created);
//   }
...
//   async findAll(filters?: MovieFilters): Promise<Movie[]> {
//     const where: Prisma.MovieWhereInput = filters?.category
//       ? { category: filters.category as any } // On utilise 'as any' pour la compatibilité des enums
//       : {};
//     const movies = await this.prisma.movie.findMany({ where });
//     return movies.map((m) => this.mapToDomain(m));
//   }
